...
from stream3r.dust3r.viz import (
    add_scene_cam,
    CAM_COLORS,
    OPENGL,
    pts3d_to_trimesh,
...
class PointCloudViewer:
    def __init__(
        ...
        self.show_camera_image = show_camera_image
    ...
    def _connect_client(self, client: viser.ClientHandle):
        # Comment out inference_step import as it doesn't exist
        # from stream3r.dust3r.inference import inference_step
        ...
        def _(_) -> None:
            # Comment out real-time inference functionality as inference_step doesn't exist
            print("Real-time inference feature is currently disabled")
            # try:
            ...
            pcs.update(
                {
                    step: {
                        ...
                        "conf": conf_list[i],
                        "edge_color": (
                            None if edge_color_list is None or edge_color_list[i] is None else edge_color_list[i]
                        ),
                    }
                }
            )
    ...
    def parse_pc_data(
        ...
        # Handle color as list or numpy array
        if isinstance(color, list):
            color = color[0]  # Extract the first element if it's a list
        ...
        if conf is not None:
            # Ensure conf has the same shape as pred_pts
            if isinstance(conf, list):
                conf = conf[0]
            conf = conf.reshape(-1)

            # Make sure mask has the same length as pred_pts
            if len(conf) != len(pred_pts):
                # If shapes don't match, skip confidence filtering
                print(f"Warning: confidence shape {conf.shape} doesn't match points shape {pred_pts.shape}, skipping confidence filtering")
            else:
        ...
        if self.downsample_factor > 1:
            pred_pts = pred_pts[::self.downsample_factor]
            color = color[::self.downsample_factor]
