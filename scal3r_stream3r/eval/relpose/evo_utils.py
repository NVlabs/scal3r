...
def load_replica_traj(gt_file):
    ...
    # Filter out non-finite poses (e.g. -inf from ScanNet tracking failures)
    valid_mask = np.isfinite(traj_w_c).all(axis=1)
    if not valid_mask.all():
        n_invalid = (~valid_mask).sum()
        print(f"Warning: {gt_file} has {n_invalid} non-finite poses, filtering them out.")
    timestamps_mat = np.arange(traj_w_c.shape[0]).astype(float)
    traj_w_c = traj_w_c[valid_mask]
    timestamps_mat = timestamps_mat[valid_mask]
