...
def rotation_matrix_to_9d(R: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        rot_9d: [..., 9] flattened rotation matrix
    """
    return R.reshape(*R.shape[:-2], 9)


def compute_gt_relative_pose(gt_extrinsics: torch.Tensor) -> tuple:
    """
    Compute ground truth relative poses between consecutive frames.

    Args:
        gt_extrinsics: [B, S, 4, 4] - camera poses (cam2world in dust3r convention)
    ...
    original_dtype = gt_extrinsics.dtype

    # Cast to float32 for matrix operations (inverse doesn't support bfloat16)
    gt_extrinsics = gt_extrinsics.float()

    # Initialize with identity for first frame
    rel_trans = torch.zeros(B, S, 3, device=device, dtype=torch.float32)
    rel_rot = torch.eye(3, device=device, dtype=torch.float32).unsqueeze(0).unsqueeze(0).expand(B, S, 3, 3).clone()

    for i in range(1, S):
        # Get camera-to-world transforms
        T_i = gt_extrinsics[:, i]  # [B, 4, 4]
        T_prev = gt_extrinsics[:, i-1]  # [B, 4, 4]

        # Relative transform: from camera i-1 to camera i
        # T_rel = T_i^(-1) @ T_prev = world2cam_i @ cam2world_prev
        # This gives the relative pose of camera i-1 in camera i's coordinate frame
        T_i_inv = torch.inverse(T_i)
        rel_T = T_i_inv @ T_prev  # [B, 4, 4]

        rel_rot[:, i] = rel_T[:, :3, :3]
        rel_trans[:, i] = rel_T[:, :3, 3]

    # Cast back to original dtype
    rel_trans = rel_trans.to(original_dtype)
    rel_rot = rel_rot.to(original_dtype)

    return rel_trans, rel_rot


def relative_pose_loss(
    pred_rel_trans: torch.Tensor,
    pred_rel_rot: torch.Tensor,
    gt_rel_trans: torch.Tensor,
    gt_rel_rot: torch.Tensor,
    ...
    Args:
        pred_rel_trans: [B, S, 3] - predicted relative translations (already scale aligned)
        pred_rel_rot: [B, S, 3, 3] - predicted relative rotation matrices (orthogonalized)
        gt_rel_trans: [B, S, 3] - ground truth relative translations (already scale aligned)
        gt_rel_rot: [B, S, 3, 3] - ground truth relative rotation matrices
        rot_loss_weight: Weight for rotation loss (default: 10.0)

    Returns:
        dict with 'loss_rel_pose', 'loss_rel_trans', 'loss_rel_rot'
    """
    # Skip first frame (identity)
    pred_trans = pred_rel_trans[:, 1:]
    pred_rot = pred_rel_rot[:, 1:]
    gt_trans = gt_rel_trans[:, 1:]
    gt_rot = gt_rel_rot[:, 1:]

    # Translation loss: L1
    trans_loss = torch.abs(pred_trans - gt_trans).mean()

    # Rotation loss: 9D L1
    pred_rot_9d = rotation_matrix_to_9d(pred_rot)  # [B, S-1, 9]
    gt_rot_9d = rotation_matrix_to_9d(gt_rot)      # [B, S-1, 9]
    ...
    # Rotation weight 10x
    total_loss = trans_loss + rot_loss_weight * rot_loss

    return {
        'loss_rel_pose': total_loss,
        'loss_rel_trans': trans_loss,
        'loss_rel_rot': rot_loss,
    }
